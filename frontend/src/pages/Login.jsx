...
export default function Login() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 relative overflow-hidden bg-brand-dark">
      {/* Background Orbs */}
      <div className="absolute top-0 right-0 w-96 h-96 bg-brand-primary/30 rounded-full blur-3xl pointer-events-none"></div>
      <div className="absolute bottom-0 left-0 w-96 h-96 bg-brand-accent/20 rounded-full blur-3xl pointer-events-none"></div>
      ...
      <div className="glass-card p-10 w-full max-w-md relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-brand-light/60">Log in to your Savvy AI dashboard</p>
        </div>
        ...
        <p className="text-center mt-8 text-brand-light/60 text-sm">
          Don't have an account? <Link to="/register" className="text-brand-secondary hover:text-brand-accent transition-colors font-semibold">Sign up for free</Link>
        </p>
      </div>
    </div>
  );
